    ...
    while True:
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to edit backup message, sending new one: {edit_e}")
                msg = await client.send_document(
                    config.STORAGE_CHANNEL,
                    document=drive_cache_path,
                    caption=caption,
                    file_name="drive.data",
                )
                # Update config with the new valid ID so subsequent edits work
...
async def loadDriveData():
    ...
    try:
        try:
            ...
            raise Exception(f"Error fetching backup message: {e}")
