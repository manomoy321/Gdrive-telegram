...
if (getCurrentPath() !== '/') {
    ...
    for (let i = 0; i < sections.length; i++) {
        sections[i].setAttribute('class', 'unselected-item');
    }

    const path = getCurrentPath();
    if (path.includes('/saved_messages')) {
        if (sections[1]) sections[1].setAttribute('class', 'selected-item');
    } else if (path.includes('/downloads')) {
        if (sections[2]) sections[2].setAttribute('class', 'selected-item');
    } else if (path.includes('/trash')) {
        if (sections[3]) sections[3].setAttribute('class', 'selected-item');
    }
}
