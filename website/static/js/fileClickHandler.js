...
function openMoreButton(div) {
    ...
    if (!isTrash) {
        ...
        try {
            const saveTgBtn = moreDiv.querySelector(`#save-tg-${id}`);
            if (saveTgBtn) {
                saveTgBtn.addEventListener('click', saveToTelegram);
            }
        }
        catch { }
    }
...
async function saveToTelegram() {
    const id = this.getAttribute('id').split('-')[2];
    const moreDiv = document.getElementById(`more-option-${id}`);
    const path = moreDiv.getAttribute('data-path') + '/' + id;
    const name = moreDiv.getAttribute('data-name');
    
    // Hide popup
    moreDiv.style.opacity = '0';
    setTimeout(() => { moreDiv.style.zIndex = '-1'; }, 300);
    
    try {
        const response = await postJson('/api/saveToSavedMessages', { 'path': path });
        if (response.status === 'ok') {
            alert(`"${name}" sent to your Telegram Saved Messages!`);
        } else {
            alert(response.message || 'Failed to save to Telegram Saved Messages');
        }
    } catch (err) {
        alert('Error saving to Telegram: ' + err);
    }
}
